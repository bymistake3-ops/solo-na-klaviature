...
> 🚀 **Хочешь просто задеплоить и начать пользоваться?**
> Читай пошаговую инструкцию в [**DEPLOY.md**](./DEPLOY.md) — 30 минут, 0 ₽, без знания Git и кода.
...
### Почему PostgreSQL через Neon
Бесплатный тир Neon (0.5 ГБ) покрывает потребности маленькой команды на годы, и
Prisma подключается к нему одной строкой. Деплой на Vercel + Neon — 30 минут, 0 ₽.

---

## Быстрый старт (разработка)

Требуется Node.js 20+ и строка подключения к PostgreSQL (например, с [Neon](https://neon.tech) — там есть бесплатный тир).

```bash
cd crm
cp .env.example .env
# 1) открой .env и впиши DATABASE_URL (из Neon) и AUTH_SECRET
# 2) установка и миграции
npm install
npm run db:push       # создать таблицы
npm run db:seed       # (опционально) залить демо-клиентов
npm run dev           # http://localhost:3001
```

Если сидов **не запускать**, при первом открытии сайта ты увидишь экран «Первый запуск» — там создашь админ-аккаунт, и можно работать со своими данными с чистого листа.

### Демо-доступ (если запустил seed)
